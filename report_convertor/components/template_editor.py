...
from report_convertor.components.template_editor_controller import TemplateEditorController
from report_convertor.components.template_editor_support import merge_destination_names, normalize_mappings, refresh_source_columns, summary_text
...
class TemplateEditorWidget(QWidget):
    ...
    def __init__(self, templates_dir: Path, status_callback) -> None:
        ...
        self.summary_label = QLabel("Create a template draft, save it, then preview or generate a report.")
        ...
        self.request_review.connect(self._review_worker.generate_review, Qt.ConnectionType.QueuedConnection)
        ...
        self._build_ui()
        self.load_draft(TemplateDraft(template_name="draft-template", output_file="output.xlsx"))
    ...
    def _build_ui(self) -> None:
        ...
        top = QHBoxLayout()
        top.addLayout(self._panel("Destination Fields", self.destination_fields, [("Add Field", self._controller.add_destination_field), ("Import Headers", self._controller.import_destination_fields), ("Remove Field", self._controller.remove_destination_field)]), 1)
        top.addLayout(self._panel("Source Reports", self.sources, [("Add Source", self._controller.add_source_file), ("Remove Source", self._controller.remove_source_file)]), 1)
        actions = QHBoxLayout()
        actions.addWidget(QLabel("Preview Rows"))
        actions.addWidget(self.preview_rows_input)
        actions.addStretch(1)
        for text, handler in [("New Draft", self._controller.new_draft), ("Save Template", self._controller.save_template), ("Preview", self._controller.preview_template), ("Generate Report", self._controller.generate_report)]:
            button = QPushButton(text)
            button.clicked.connect(handler)
            actions.addWidget(button)
        layout = QVBoxLayout()
        for item in [form, self.summary_label, top, QLabel("Mappings"), self.mapping_editor, actions, QLabel("Preview"), self.preview_table]:
            layout.addLayout(item) if isinstance(item, (QFormLayout, QHBoxLayout)) else layout.addWidget(item)
        self.setLayout(layout)

    def _file_row(self) -> QWidget:
        return self._button_row(self.template_path_input, [("Browse", self._controller.browse_template), ("Load", self._controller.load_template)])

    def _panel(self, title: str, widget: QWidget, buttons: list[tuple[str, object]]) -> QVBoxLayout:
        layout = QVBoxLayout()
    ...
    def _button_row(self, first_widget: QWidget | None, buttons: list[tuple[str, object]]) -> QWidget:
    ...
    def load_draft(self, draft: TemplateDraft) -> None:
        ...
        self.destination_fields.load_fields([field.name for field in draft.destination_fields])
        self.sources.load_sources(draft.sources)
    ...
    def compose_draft(self, require_name: bool) -> TemplateDraft:
        name = self.template_name_input.text().strip() or ("draft-template" if not require_name else "")
        names = self.destination_fields.field_names()
        sources = self.sources.sources()
        mappings = normalize_mappings(self.mapping_editor.read_mappings(), names, sources)
        return TemplateDraft(template_name=name, output_file=self.output_file_input.text().strip() or "output.xlsx", destination_fields=[DestinationField(name=name) for name in names], sources=sources, mappings=mappings).ensure_mapping_rows()

    def merge_imported_fields(self, imported_fields) -> None:
        names = self.destination_fields.field_names()
        self.destination_fields.load_fields(merge_destination_names(names, [field.name for field in imported_fields]))
        self.sync_editor()
    ...
    def summary(self) -> str:
        return summary_text(self.template_name_input.text().strip(), self.compose_draft(require_name=False))
    ...
    def _dispatch_review_request(self) -> None:
        ...
        if sum(1 for mapping in draft.mappings if mapping.is_complete) < 1:
            self.preview_table.load_rows([])
            self.show_status("Review preview will appear after at least one field is mapped.")
            return
        self._review_request_id += 1
        self.show_status("Refreshing review preview...")
        self.request_review.emit(self._review_request_id, draft, self.preview_rows_input.value())

    @pyqtSlot(int, list)
    def _handle_review_ready(self, request_id: int, rows: list[dict[str, object]]) -> None:
